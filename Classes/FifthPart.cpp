...
bool FifthPart::init() {
	...
	gameLayer = (Layer*)CSLoader::createNode("FifthPart.csb");
	...
	//设置监听器的碰撞开始函数  
	contactListener->onContactSeparate = CC_CALLBACK_1(FifthPart::onContactSeperate, this);
	contactListener->onContactBegin = CC_CALLBACK_1(FifthPart::onContactBegin, this);
	//添加到事件分发器中  
...
void FifthPart::update(float dt) {
	if ((!isGameOver) && (!isWin))
	{
...
void FifthPart::addBall(float x, float y) {
	ball = Ball::create();
...
bool FifthPart::onContactBegin(const PhysicsContact& contact)
{
	return true;
}

void FifthPart::onContactSeperate(PhysicsContact& contact) {
...
void FifthPart::collusion(Sprite* spriteA, Sprite* spriteB) {
	if (!spriteA || !spriteB)
	{
		return;
	}
