...
Scene* SecondPart::createScene() {
	...
	auto layer = SecondPart::create();
	layer->setPhysicsWorld(scene->getPhysicsWorld());
	scene->addChild(layer);
	return scene;
}

bool SecondPart::init() {
	...
	//设置监听器的碰撞开始函数  
	contactListener->onContactSeparate = CC_CALLBACK_1(SecondPart::onContactSeperate, this);
	contactListener->onContactBegin = CC_CALLBACK_1(SecondPart::onContactBegin, this);
	//添加到事件分发器中  
...
void SecondPart::update(float dt) {
	if ((!isGameOver) && (!isWin))
	{
...
void SecondPart::addBall(float x, float y) {
	ball = Ball::create();
...
bool SecondPart::onContactBegin(const PhysicsContact& contact)
{
	return true;
}

void SecondPart::onContactSeperate(PhysicsContact& contact) {
...
void SecondPart::collusion(Sprite* spriteA, Sprite* spriteB) {
	if (!spriteA || !spriteB)
	{
		return;
	}
