...
bool ForthPart::init() {
	...
	gameLayer = (Layer*)CSLoader::createNode("ForthPart.csb");
	...
	//设置监听器的碰撞开始函数  
	contactListener->onContactSeparate = CC_CALLBACK_1(ForthPart::onContactSeperate, this);
	contactListener->onContactBegin = CC_CALLBACK_1(ForthPart::onContactBegin, this);
	//添加到事件分发器中  
...
void ForthPart::update(float dt) {
	if ((!isGameOver) && (!isWin))
	{
...
void ForthPart::addBall(float x, float y) {
	ball = Ball::create();
...
bool ForthPart::onContactBegin(const PhysicsContact& contact)
{
	return true;
}

void ForthPart::onContactSeperate(PhysicsContact& contact) {
...
void ForthPart::collusion(Sprite* spriteA, Sprite* spriteB) {
	if (!spriteA || !spriteB)
	{
		return;
	}
...
void ForthPart::addWinLayer() {
	...
	auto backButton = Sprite::create("Button/Button_Back.png");
	backButton->setAnchorPoint(Vec2(0, 0));
	backButton->setPosition(550, 146);
