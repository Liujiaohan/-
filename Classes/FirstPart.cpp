...
bool FirstPart::init() {
	...
	gameLayer = (Layer*)CSLoader::createNode("FirstPart.csb");
	...
	//设置监听器的碰撞开始函数  
	contactListener->onContactSeparate = CC_CALLBACK_1(FirstPart::onContactSeperate, this);
	contactListener->onContactBegin = CC_CALLBACK_1(FirstPart::onContactBegin, this);
	//添加到事件分发器中  
...
void FirstPart::addEdge() {
	auto leftedge = Sprite::create();
	PhysicsBody* leftboundBody = PhysicsBody::createEdgeBox(Size(2, visibleSize.height), PHYSICSBODY_MATERIAL_DEFAULT);
	leftboundBody->setDynamic(false);
	...
	PhysicsBody* rightboundBody = PhysicsBody::createEdgeBox(Size(2, visibleSize.height), PHYSICSBODY_MATERIAL_DEFAULT);
	rightboundBody->setDynamic(false);
	...
	PhysicsBody* upboundBody = PhysicsBody::createEdgeBox(Size(visibleSize.width, 2), PHYSICSBODY_MATERIAL_DEFAULT);
	upboundBody->setDynamic(false);
...
void FirstPart::update(float dt) {
	if ((!isGameOver) && (!isWin))
	{
	...
	else if (isWin)
	{
		...
		if ((ball->getPosition().y<0) && (bricks.size() != 0))
		{
			isGameOver = true;
			//ball->release();
...
void FirstPart::addBall(float x, float y) {
	ball = Ball::create();
	...
	//设置物体的摩擦力  
	ballBodyOne->getShape(0)->setFriction(0.5f);
	//设置物体密度  
...
bool FirstPart::onContactBegin(const PhysicsContact& contact)
{
	return true;
}

void FirstPart::onContactSeperate(PhysicsContact& contact) {
...
void FirstPart::collusion(Sprite* spriteA, Sprite* spriteB) {
	...
	if ((tagA == 1) || (tagA == 2) || (tagA == 3) || (tagA == 4) || (tagA == 5))
	{
	...
	else if ((tagB == 1) || (tagB == 2) || (tagB == 3) || (tagB == 4) || (tagB == 5))
	{
		spriteB->removeFromParent();
		audio->playEffect("kit_brick.wav", false, 1.0f, 1.0f, 1.0f);
		auto sun = ParticleExplosion::create();
		sun->setTexture(Director::getInstance()->getTextureCache()->addImage("star.png"));
		sun->setPosition(spriteA->getPosition());
	...
	//	score += 2;
	//	bricks.eraseObject(spriteA);
	//} else if ((tagB == 0) && ((tagA==3)||(tagA==4)||(tagA==5)))
	//{
	//	audio->playEffect("kit_brick.wav", false, 1.0f, 1.0f, 1.0f);
...
void FirstPart::addPaddle() {
	...
	Point points[5] = { Point(-94.00000, -40.00000), Point(-94.00000, 40.00000),Point(-50.00000,40.00000),Point(0.00000,0.00000),
		Point(0.00000,-40.00000)};
	Point points2[5] = {Point(0.00000,-40.00000), Point(0.00000,0.00000),Point(50.00000,40.00000) , Point(94.00000, 40.00000), Point(94.00000, -40.00000)};
	auto body1 = PhysicsShapePolygon::create(points, 5);
	auto body2 = PhysicsShapePolygon::create(points2,5);
	//Point points[4] = { Point(-30.00000, -30.50000), Point(0.00000, 30.50000), Point(10.00000, 30.50000),Point(30.00000, -30.50000) };
	...
	case 6:
		...
		break;
	case 8:
		paddleBody = PhysicsBody::create();
		paddleBody->addShape(body1);
		paddleBody->addShape(body2);
	
	//	paddleBody = PhysicsBody::createBox(Size(paddle->getContentSize().width, paddle->getContentSize().height), PhysicsMaterial(0.1f, 1.0f, 0.0f));
		break;
	default:
	...
	//设置物体的摩擦力  
	paddleBody->getShape(0)->setFriction(0.5f);
	//设置物体密度  
...
void FirstPart::addWinLayer() {
	...
	auto backButton = Sprite::create("Button/Button_Back.png");
	backButton->setAnchorPoint(Vec2(0, 0));
	backButton->setPosition(558, 146);
	...
	goonButton->setPosition(210, 146);
